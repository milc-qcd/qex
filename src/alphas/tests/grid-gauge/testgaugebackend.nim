...
when isMainModule:
  ...
  proc testGrid(test: TestType) =
    ...
    of ActionTest:
      ...
      echo "QEX:  ", qexResult
      echo "ratio: ", altGridResult / qexResult
      echo "diff: ", altGridResult - qexResult
    of ForceTest:
      qexGauge.random()
      gridGauge := qexGauge
      ...
      altGridAction.gaugeForce2(gridGauge, gridForce)
      qexAction.gaugeForce2(qexGauge, qexForce)

      gridForceQEX := gridForce

      threads:
        for i in 0..<gridForceQEX.len:
          gridForceQEX[i] *= 2.0 #:= 2.0*tempGridForceQEX[i]

      #let gridResult = qexAction.gaugeAction2(gridForceQEX)
      var gridResult = qexAction.gaugeAction2(gridForceQEX)
      let qexResult = qexAction.gaugeAction2(qexForce)

      #echo "Grid: ", gridResult
      echo "Grid (alt, no parallelogram): ", gridResult
      echo "QEX:  ", qexResult
