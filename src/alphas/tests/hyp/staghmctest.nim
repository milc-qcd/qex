...
# !!!!!!!!!!!!!!!!!! REMEMBER TO COMPILE W/ FUELCOMPAT !!!!!!!!!!!!!!!!!!
...
letParam:
  ...
  gintalg:IntProc = "2MN"
  gsteps = 10
  mass = @[0.005]  # mass for each staggered species
  hmasses0 = @[0.6]  # Hasenbusch masses for mass[0]
  ...
  fintalg:IntProc = "2MN"
  fsteps = repeat(10, mass.len)  # nsteps for each mass
...
let
  ...
  gc = GaugeActionCoeffs(plaq: beta*Cp, rect: beta*Cr)
  vol = lo.physVol
...
proc gaction(g:auto, f2:seq[seq[float]], p2:float):auto =
  ...
  u.unit
  let
    ga = gc.gaugeAction1(g) - gc.gaugeAction1(u)
    fa = f2.mapit(0.5*it)
...
proc mdvAll(dtau: openarray[float]) =
  ...
  if (dtauG != 0.0): mdv dtauG
  if (dtauF != 0.0): 
    var sforce = g.smearRephase sg
    mdvf(@[0, 1], sforce, @[dtauF, dtauF])

let
  (V,T) = newIntegratorPair(mdvAll, mdt)
  VG = gintalg(steps = gsteps, V = V[0], T = T)
  VF = V[1]
let H = fintalg(steps = fsteps[0], V = VF, T = VG)
