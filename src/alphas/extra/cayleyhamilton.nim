...
template rsqrtPHM3(r:typed; x:typed; eps:typed) =
  let tr = trace(x).re
  let x2 = x*x
  let p2 = trace(x2).re
  let det = determinant(x).re
  var c0,c1,c2:type(tr)
  rsqrtPHM3f(c0, c1, c2, tr, p2, det, eps)
  r := c0 + c1*x + c2*x2
...
template rsqrtPHM(r:typed; x:typed; eps:typed) =
  mixin rsqrt, nrows
  ...
  elif r.nrows == 3: rsqrtPHM3(r, x, eps)
  else: rsqrtPHMN(r, x)
