...
proc asqtadDeriv[T](
  ...
  threads: # Feb. 19, 2026 fix to bug introduced in commit 6ef606d of main repo
    for mu in 0..<mid.len:
      f[mu] := 0
      fll[mu] := 0
...
proc project[T](self: UnitaryProjection, v: auto; u: T) =
  threads:
    for mu in 0..<u.len:
      for s in u[mu]: self.projectU(v[mu][s], u[mu][s], false)
...
proc projectDeriv[T](
  ...
  threads:
    for mu in 0..<chain.len:
      for s in chain[mu]:
        self.projectUderiv(dvdu[mu][s], v[mu][s], u[mu][s], chain[mu][s], false)
...
proc projectDeriv[T](
  ...
  threads:
    for mu in 0..<chain.len:
      for s in chain[mu]:
        self.projectUderiv(dvdu[mu][s], u[mu][s], chain[mu][s], true)
...
proc smearGetForce*[T](
  ...
  proc smearedForce(dsdu: var T; dsdsu,dsdsul: T) =
    ...
    if not regulate: self.projection.projectDeriv(t, w, v, t)
    else: self.projection.projectDeriv(t, v, t)
